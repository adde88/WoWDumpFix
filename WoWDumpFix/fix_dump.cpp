...
bool fixdump::current::FixWoW()
{
    ...
    if (!GetWoWPeHeader(peHeader)) {
        pluginLog("Error: failed to get WoW's PE header.\n");
        return false;
    }
    ...
    // Make WoW's PE header, .text, and .rdata regions writable.
...
//
BOOL fixdump::current::GetWoWImageSize(HANDLE hProcess, PULONG pcbImageSize)
{
    PVOID pPeb = NULL;
    PVOID pPebLdr = NULL;
    ULONG_PTR PebLdr = 0;
    ...
    PLDR_DATA_TABLE_ENTRY pWoWLdrDataEntry = NULL;
    LDR_DATA_TABLE_ENTRY WoWLdrDataEntry = {};
    BOOL status = TRUE;
    ...
    *pcbImageSize = 0;
    ...
    //
    if (!WoWLdrDataEntry.SizeOfImage)
    {
    ...
    DBG_PRINT("WoWLdrDataEntry.SizeOfImage:   0x%IX\n",
        WoWLdrDataEntry.SizeOfImage);

    // Set out parameters.
    *pcbImageSize = WoWLdrDataEntry.SizeOfImage;
...
bool fixdump::current::GetWoWPeHeader(BUFFERED_PE_HEADER& PeHeader)
{
    ...
    if (!GetModuleFileNameExW(debuggee.hProcess,
                              ...
                              wowPath,
                              MAX_MODULE_SIZE)) {
        pluginLog("Error: failed to get WoW's path.\n");
        return false;
    }
    ...
    if (hWoWFile == INVALID_HANDLE_VALUE) {
        pluginLog("Error: failed to open Wow.exe while getting PE header.\n");
        return false;
    }
    ...
    if (!ReadFile(hWoWFile,
                  ...
                  &numBytesRead,
                  nullptr)) {
        pluginLog("Error: failed to read Wow.exe.\n");
...
SIZE_T BuildNewWoWRegion(const REMOTE_PE_HEADER& WoWPEHeader)
{
    ...
    if (const SIZE_T secretPEHeaderAddress = GetSecretPEHeaderBaseAddress())
    {
                                 ...
                                 WoWPEHeader.optionalHeader->SizeOfImage))
        {
            ...
            pluginLog("BuildNewWoWRegion: Failed to write transfer buffer to Wow.exe.\n");
    }
    else
...
bool NoticeMeScylla(const REMOTE_PE_HEADER& NewRegionPEHeader)
{
    ...
    // read Wow.exe's LDR_DATA_TABLE_ENTRY.
